...
#ifndef FILE_NSSF_PROFILE_HPP_SEEN
#define FILE_NSSF_PROFILE_HPP_SEEN
...
class nssf_profile : public std::enable_shared_from_this<nssf_profile> {
public:
  nssf_profile()
  ...
  nssf_profile(const std::string &id)
  ...
  nssf_profile &operator=(const nssf_profile &s) {
  ...
  // nssf_profile(nssf_profile &b) = delete;

  virtual ~nssf_profile() {
    // Logger::nssf_app().debug("Delete NSSF Profile instance...");
  }
  ...
  /*
   * Covert from a json represetation to NSSF profile
   * @param [nlohmann::json &] data: Json data
   * @return void
   */
