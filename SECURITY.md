...
- **Email subject**: `[SEC-VUL-OAI-CN5G-NSSF]: "Mention the affected
  functionality here"`.
...
in the
[GitHub Issue Tracker](https://github.com/openairinterface/oai-cn5g-nssf/issues),
...
- **Issue title**: `[AI-SEC-VUL-OAI-CN5G-NSSF]: "Mention the affected
  functionality here"`.
- **Issue label**: apply the
  https://github.com/openairinterface/oai-cn5g-nssf/labels/security
...
The [openairinterface/oai-cn5g-nssf](https://github.com/openairinterface/oai-cn5g-nssf)
repository contains the OAI implementation of the 5G Core **Network Slice
Selection Function (NSSF)**, together with its configuration templates,
...
NRF, PCF, LMF, NEF, NWDAF), in the federation repository
...
can be reported here if they are reachable through the NSSF. In that case,
please state clearly which component is affected and give the commit ID of the
affected submodule in addition to the NSSF commit.

Security reports are in scope when they affect the confidentiality, integrity,
or availability of the NSSF running in a documented or reasonably expected
deployment.
...
- Memory corruption, crashes, infinite loops, resource exhaustion, or denial of
  ...
  requests, or by malformed YAML in the configuration and slice-configuration
  files parsed at startup.
- Defects in the implemented NSSF service operations (the `NSSelection` Get
  operation, the `NSSAIAvailability` PUT update, and the OAI
  slice-configuration API-list endpoint) that allow a consumer to obtain slice
  selection information without appropriate authorisation, to register or
  replace the S-NSSAI availability recorded for an AMF it does not own, or
  otherwise to influence the selection result so that a UE registration or PDU
  session is steered towards an unintended slice, Network Slice Instance, or
  AMF Set (3GPP TS 23.501, TS 29.531).
- Exposure of network topology or slice deployment information, such as
  configured S-NSSAIs, Network Slice Instance identifiers, TAI or PLMN
  mappings, AMF Set identifiers, or the NRF endpoints and management URIs
  returned in `NsiInformation`, through logs, statistics, API responses, or
  error paths.
...
- Bugs requiring local admin/root access on the NSSF host with no privilege
  boundary crossed.
...
- Vulnerabilities solely caused by third-party projects or dependencies should
  ...
  the NSSF deployment or requires a change in this repository, please report it
  here as well.
- Attacks that require a trusted operator role, such as an already-authorised
  AMF or peer NF deliberately misconfigured by the deployment owner, unless a
...
- Denial of service that only depends on flooding a plaintext, unprotected
  ...
  deployment, rather than on an NSSF software defect.
- Features documented as not supported in
  [FEATURE_SET.md](./docs/FEATURE_SET.md), including the NSSAI availability
  delete, patch and options endpoints and the event subscription endpoints,
  which currently answer with `503 API NOT IMPLEMENTED`, or issues in
  experimental or incomplete functionality, unless they demonstrate a realistic
