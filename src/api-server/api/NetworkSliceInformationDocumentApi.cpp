...
const std::string NetworkSliceInformationDocumentApi::base = "/nnssf-nsselection/v2";

NetworkSliceInformationDocumentApi::NetworkSliceInformationDocumentApi(const std::shared_ptr<Pistache::Rest::Router>& rtr)
    : router(rtr)
{
}
...
void NetworkSliceInformationDocumentApi::setupRoutes() {
    ...
    Routes::Get(*router, base + "/network-slice-information", Routes::bind(&NetworkSliceInformationDocumentApi::n_s_selection_get_handler, this));

    // Default handler, called when a route is not found
    router->addCustomHandler(Routes::bind(&NetworkSliceInformationDocumentApi::network_slice_information_document_api_default_handler, this));
}

std::pair<Pistache::Http::Code, std::string> NetworkSliceInformationDocumentApi::handleParsingException(const std::exception& ex) const noexcept
{
    try {
        throw;
...
std::pair<Pistache::Http::Code, std::string> NetworkSliceInformationDocumentApi::handleOperationException(const std::exception& ex) const noexcept
{
    return std::make_pair(Pistache::Http::Code::Internal_Server_Error, ex.what());
}

void NetworkSliceInformationDocumentApi::n_s_selection_get_handler(const Pistache::Rest::Request &request, Pistache::Http::ResponseWriter response) {
    try {
    ...
    auto sliceInfoRequestForRegistrationQuery = request.query().get("slice-info-request-for-registration");
    ...
    if(!sliceInfoRequestForRegistrationQuery.isEmpty()){
        ...
        // if(fromStringValue(sliceInfoRequestForRegistrationQuery.get(), valueQuery_instance)){
        //     sliceInfoRequestForRegistration = Pistache::Some(valueQuery_instance);
        // }
    }
    auto sliceInfoRequestForPduSessionQuery = request.query().get("slice-info-request-for-pdu-session");
    ...
    if(!sliceInfoRequestForPduSessionQuery.isEmpty()){
        ...
        // if(fromStringValue(sliceInfoRequestForPduSessionQuery.get(), valueQuery_instance)){
        //     sliceInfoRequestForPduSession = Pistache::Some(valueQuery_instance);
        // }
    }
    auto sliceInfoRequestForUeCuQuery = request.query().get("slice-info-request-for-ue-cu");
    ...
    if(!sliceInfoRequestForUeCuQuery.isEmpty()){
        ...
        // if(fromStringValue(sliceInfoRequestForUeCuQuery.get(), valueQuery_instance)){
    ...
    try {
        this->n_s_selection_get(nfType, nfId, sliceInfoRequestForRegistration, sliceInfoRequestForPduSession, sliceInfoRequestForUeCu, homePlmnId, tai, supportedFeatures, response);
...
void NetworkSliceInformationDocumentApi::network_slice_information_document_api_default_handler(const Pistache::Rest::Request &, Pistache::Http::ResponseWriter response) {
